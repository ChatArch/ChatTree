...
def test_tree_reports_registered_root_only_surface():
    ...
    assert result.output.splitlines().count("chattree") == 1
...
def test_tree_brief_reports_the_same_root_only_nodes():
    ...
    assert brief.output.splitlines().count("chattree") == 1
...
def test_tree_brief_omits_registered_command_signatures():
    ...
    def inspect(path: str) -> None:
        """Inspect a tree path; read-only text output."""
    ...
    assert "inspect  # Inspect a tree path; read-only text output." in brief.output
